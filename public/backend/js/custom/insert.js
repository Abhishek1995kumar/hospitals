    ...
    function loadData(type, url) {
        $.ajax({
            ...
            success:function(response) {
                let data = typeof response.data.system === 'string' ? JSON.parse(response.data.system) : response.data.system;
                let data1 = typeof response.data.customer === 'string' ? JSON.parse(response.data.customer) : response.data.customer;
                let planData = typeof response.data.data === 'string' ? JSON.parse(response.data.data) : response.data.data;
                let parentModules = typeof response.data.parentModules === 'string' ? JSON.parse(response.data.parentModules) : response.data.parentModules;
                let childModules = typeof response.data.childModules === 'string' ? JSON.parse(response.data.childModules) : response.data.childModules;
                let html = '';
                let x = 1;
                // Role, Permission, Role Permission, User Role Table
                if(type == 'role'){
                    if (response.data.system && response.data.system.length > 0) {
                        data.forEach(function(role) {
                            html += `<tr>
                                    <td class="text-center">${x++}</td>
                                    <td class="text-center">${role.name}</td>
                                    <td class="text-center">${role.scope == 0 ? 'System' : 'Customer'}</td>
                                    <td class="text-center">${role.role_priority}</td>
                                    <td class="text-center">
                                        <button class="btn btn-sm btn-primary" onclick='editRole(${JSON.stringify(role)})'>Edit</button>
                    ...
                    } else if (response.data.customer && response.data.customer.length > 0) {
                        data1.forEach(function(role) {
                            html += `<tr>
                                    <td class="text-center">${x++}</td>
                                    <td class="text-center">${role.name}</td>
                                    <td class="text-center">${role.scope}</td>
                                    <td class="text-center">${role.role_priority}</td>
                                    <td class="text-center">
                                        <button class="btn btn-sm btn-primary" onclick='editRole(${JSON.stringify(role)})'>Edit</button>
                                        <button class="btn btn-sm btn-danger" onclick="deleteRole('${role.id}')">Delete</button>
                ...
                if(type == 'module') {
                    if (parentModules && parentModules.length > 0) {
                        parentModules.forEach(function(permission) {
                            html += `<tr>
                                    <td class="text-center">${x++}</td>
                                    <td class="text-center">${permission.name}</td>
                                    <td class="text-center">
                                        <button class="btn btn-sm btn-primary" onclick='editPermission(${JSON.stringify(permission)})'>Edit</button>
                ...
                if(type == 'child-module') {
                    if (childModules && childModules.length > 0) {
                        childModules.forEach(function(permission) {
                            html += `<tr>
                                    <td class="text-center">${x++}</td>
                                    <td class="text-center">${permission.parent_name}</td>
                                    <td class="text-center">${permission.name}</td>
                                    <td class="text-center">
                                        <button class="btn btn-sm btn-primary" onclick='editPermission(${JSON.stringify(permission)})'>Edit</button>
                                        <button class="btn btn-sm btn-danger" onclick="deletePermission('${permission.id}')">Delete</button>
                        ...
                        $('#childModuleTable tbody').html(html);
                    }
                } 
                if(type == 'permission') {
                    if (response.data.system && response.data.system.length > 0) {
                        data.forEach(function(permission) {
                            html += `<tr>
                                    <td class="text-center">${x++}</td>
                                    <td class="text-center">${permission.modules_name}</td>
                                    <td class="text-center">${permission.name}</td>
                    ...
                    if (response.data.system && response.data.system.length > 0) {
                        data.forEach(function(permission) {
                            html += `<tr>
                                    ...
                                    <td class="text-center">${permission.permission_action}</td>
                                    <td class="text-center">
                                        <button class="btn btn-sm btn-primary" onclick='editPermission(${JSON.stringify(permission)})'>Edit</button>
                                        <button class="btn btn-sm btn-danger" onclick="deletePermission('${permission.id}')">Delete</button>
                                    </td>
                        ...
                        data1.forEach(function(permission) {
                            html += `<tr>
                                    <td class="text-center">${x++}</td>
                                    <td class="text-center">${permission.role_name}</td>
                                    <td class="text-center">${permission.permission_action}</td>
                ...
                if(type == 'roleUser') {
                    if (response.data.system && response.data.system.length > 0) {
                        data.forEach(function(userRole) {
                            html += `<tr>
                                    <td class="text-center">${x++}</td>
                                    <td class="text-center">${userRole.role_name}</td>
                                    <td class="text-center">${userRole.fname} ${userRole.lname}</td>
                                    <td class="text-center">
                                        <button class="btn btn-sm btn-primary" onclick='editPermission(${JSON.stringify(userRole)})'>Edit</button>
                                        <button class="btn btn-sm btn-danger" onclick="deletePermission('${userRole.id}')">Delete</button>
                                    </td>
                                </tr>`;
                        });
                        ...
                        data1.forEach(function(userRole) {
                            html += `<tr>
                                    <td class="text-center">${x++}</td>
                                    <td class="text-center">${userRole.role_name}</td>
                                    <td class="text-center">${userRole.fname} ${userRole.lname}</td>
                ...
                // Plan, Feature, Feature Plan Table
                if(type == 'plan') {
                    if (planData && planData.length > 0) {
                        planData.forEach(function(plan) {
                            html += `<tr>
                                    ...
                                    <td class="text-center">${plan.duration_days}</td>
                                    <td class="text-center">${plan.max_hospitals}</td>
                                    <td class="text-center">${plan.max_firms}</td>
                                    <td class="text-center">${plan.max_users}</td>
                                    <td class="text-center">
                                        <button class="btn btn-sm btn-primary" onclick='editPermission(${JSON.stringify(plan)})'>Edit</button>
                                        <button class="btn btn-sm btn-danger" onclick="deletePermission('${plan.id}')">Delete</button>
                                    </td>
                                </tr>`;
                        });
                ...
                if(type == 'feature') {
                    if (planData && planData.length > 0) {
                        planData.forEach(function(feature) {
                            html += `<tr>
                                    <td class="text-center">${x++}</td>
                                    <td class="text-center">${feature.module_name}</td>
                                    <td class="text-center">${feature.feature_name}</td>
                                    <td class="text-center">
                                        <button class="btn btn-sm btn-primary" onclick='editPermission(${JSON.stringify(feature)})'>Edit</button>
                                        <button class="btn btn-sm btn-danger" onclick="deletePermission('${feature.id}')">Delete</button>
                                    </td>
                                </tr>`;
                        });
                ...
                if(type == 'planFeature') {
                    if(planData && planData.length > 0) {
                        planData.forEach(function(featurePlan) {
                            html += `<tr>
                                    <td class="text-center">${x++}</td>
                                    <td class="text-center">${featurePlan.plan_name}</td>
                                    <td class="text-center">${featurePlan.feature_name}</td>
                                    <td class="text-center">
                                        <button class="btn btn-sm btn-primary" onclick='editPermission(${JSON.stringify(featurePlan)})'>Edit</button>
                                        <button class="btn btn-sm btn-danger" onclick="deletePermission('${featurePlan.id}')">Delete</button>
                                    </td>
                                </tr>`;
                        });
                        $('#featurePlanTable tbody').html(html);
                    }
                }
            }
        });
