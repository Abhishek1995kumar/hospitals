...
@section('title') {{ __('Pharmacy Vendor')}} @endsection
...
@section('breadcrumb')
    <h1 class="d-flex flex-column text-dark fw-bold fs-3 mb-0">{{ __('Pharmacy Vendors')}}</h1>
    <ul class="breadcrumb breadcrumb-separatorless fw-semibold fs-7 pt-1">
        ...
        <li class="breadcrumb-item text-dark">{{ __('Pharmacy Vendor')}}</li>
    </ul>
@endsection
        ...
        <div class="row">
            <div class="col-3">
                <div class="alert alert-success text-center border border-success">
                    <h5 class="alert-heading">{{ __('Total Pharmacy Vendor')}}</h5>
            ...
            <div class="card-header border-0 pt-6">
                <ul class="nav nav-tabs" id="partyTabMenu" role="tablist">
                    ...
                    <li class="nav-item" role="presentation">
                        <button class="nav-link active" id="partyTabBtn" data-bs-toggle="tab" data-bs-target="#partyTab" type="button" role="tab"> {{ __('Vendor Dashboard') }} </button>
                    </li>
                    @endpermission
                </ul>
            </div>
                    ...
                    <div class="tab-pane fade show active" id="partyTab" role="tabpanel">
                        <div class="card-header border-0 pt-6">
                            ...
                            <div class="card-toolbar">
                                <button type="button" class="btn btn-primary mx-2" id="addPharmacyVendor" data-url="{{ route('pharmacy.vendor.create') }}">
                                    {{ __('Add Pharmacy Vendor')}}
                                </button>
...
@section('scripts')
    ...
    <script>
        loadPage("{{ route('pharmacy.vendor') }}");
        $(document).off('click', '#addPharmacyVendor').on('click', '#addPharmacyVendor', function(e){
            e.preventDefault();
            loadPage("{{ route('pharmacy.vendor.create') }}");
        });
