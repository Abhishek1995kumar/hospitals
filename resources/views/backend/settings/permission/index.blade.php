...
@section('breadcrumb')
    <h1 class="d-flex flex-column text-dark fw-bold fs-3 mb-0">{{ __('Authentication')}}</h1>
    <ul class="breadcrumb breadcrumb-separatorless fw-semibold fs-7 pt-1">
        ...
        <li class="breadcrumb-item text-dark">{{ __('Authentication')}}</li>
    </ul>
@endsection
        ...
        <div class="card-header border-0 pt-6">
            <ul class="nav nav-tabs" id="roleTabMenu" role="tablist">
                ...
                <li class="nav-item" role="presentation">
                    <button class="nav-link active" id="roleTabBtn" data-bs-toggle="tab" data-bs-target="#roleTab" type="button" role="tab"> {{ __('Role Dashboard') }} </button>
                </li>
                @endpermission
                ...
                @permission('module.view')
                <li class="nav-item" role="presentation">
                    <button class="nav-link" id="childModuleTabBtn" data-bs-toggle="tab" data-bs-target="#childModuleTab" type="button" role="tab"> {{ __('Child Module') }} </button>
                </li>
                @endpermission
                                        ...
                                        <div class="card-body">
                                            ...
                                            <div class="row">
                                                <div class="col-md-12 mb-4 mt-3" id="ngoCenterDiv">
                                                    <label class="required fs-6 fw-semibold mb-2">{{ __('Permission')}}</label>
                                                    <div class="form-group">
                                                        @if(isset($permissions['system']) && $permissions['system']->isNotEmpty())
                                                            <table class="table table-bordered">
                                                                ...
                                                                <tbody>
                                                                    @foreach($permissions['system'] as $module => $perms)
                                                                        <tr>
                                                                                ...
                                                                                <div class="d-flex flex-wrap gap-4">
                                                                                    @foreach($perms as $perm)
                                                                                        <div class="form-check">
                                                                                            <input class="form-check-input" type="checkbox" value="{{ $perm->id }}" id="permission_{{ $perm->id }}" name="permission_id[]">
                                                ...
                                                <div class="col-md-6 mb-4" id="userRoleMappingDiv">
                                                    <div class="form-group">
                                                        ...
                                                        <select name="user_id[]" class="form-select search" id="userMappingId" multiple data-control="select2" data-placeholder="Select employee">
                                                            @if(isset($users['system']) && $users['system']->isNotEmpty())
                                                                @foreach($users['system'] as $user)
                                                                    @if(!in_array($user->user_type, [1,2])) <!-- super admin(top level=1) ya admin -->
                                                                    @else
                                                                        <option value="{{ $user->id }}"> {{ $user->fname }} {{ $user->lname }} </option>
                                                                    @endif
                                                                @endforeach
                                                            @elseif(isset($users['customer']) && $users['customer']->isNotEmpty())
                                                                @foreach($users['customer'] as $user)
                                                                    @if(!in_array($user->user_type, [3,4])) <!-- customer admin(top level=3) ya customer hospital admin -->
                                                                    @else
                                                                        <option value="{{ $user->id }}"> {{ $user->fname }} {{ $user->lname }} </option>
                                                                    @endif
                                                                @endforeach
